...
if (Get-Command "cl.exe" -ErrorAction SilentlyContinue) {
    ...
    cmd /c "`"$vcvars`" >nul && cl /nologo /O2 /W3 /D_CRT_SECURE_NO_WARNINGS /Fe:bin\voltbill.exe src\main.c src\utils.c src\ui.c src\banner.c src\tariff.c src\customer.c src\billing.c src\payment.c src\analytics.c src\storage.c"
    ...
    & clang -O2 -Wall -std=c99 -o bin/voltbill.exe src/main.c src/utils.c src/ui.c src/banner.c src/tariff.c src/customer.c src/billing.c src/payment.c src/analytics.c src/storage.c -lm
} else {
