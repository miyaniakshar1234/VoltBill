/**
 * @file storage.c
 * @brief Implementation of binary data storage, CSV reporting, and demo seeder.
...
int storage_create_backup(char *out_path, size_t max_len) {
    ...
    snprintf(backup_file, sizeof(backup_file), "data/backups/voltbill_backup_%s.json", timestamp);

    int res = storage_export_json(backup_file);
    if (res && out_path && max_len > 0) {
        strncpy(out_path, backup_file, max_len - 1);
        out_path[max_len - 1] = '\0';
        audit_log("DATABASE_BACKUP", backup_file);
    }
    return res;
}
...
int storage_export_json(const char *filepath) {
    ...
    fprintf(fp, "{\n");
    fprintf(fp, "  \"generator\": \"VoltBill Systems Engine\",\n");
    fprintf(fp, "  \"developer\": \"Akshar Miyani\",\n");
    fprintf(fp, "  \"version\": \"1.1.0\",\n");

    /* Export Consumers */
    fprintf(fp, "  \"consumers\": [\n");
    int c_count = customer_get_count();
    for (int i = 0; i < c_count; i++) {
        ...
        fprintf(fp, "      \"meter_no\": \"%s\",\n", c->meter_no);
        fprintf(fp, "      \"category\": \"%s\",\n", category_to_string(c->category));
        fprintf(fp, "      \"phase\": %d,\n", c->phase);
        fprintf(fp, "      \"sanctioned_load_kw\": %.2f,\n", c->sanctioned_load_kw);
        fprintf(fp, "      \"solar_capacity_kw\": %.2f,\n", c->solar_capacity_kw);
        fprintf(fp, "      \"outstanding_arrears\": %.2f,\n", c->outstanding_arrears);
        fprintf(fp, "      \"advance_credit\": %.2f,\n", c->advance_credit);
    ...
    /* Export Bills */
    fprintf(fp, "  \"bills\": [\n");
    int b_count = billing_get_count();
    for (int i = 0; i < b_count; i++) {
        BillBreakdown *b = billing_get_by_index(i);
        fprintf(fp, "    {\n");
        fprintf(fp, "      \"bill_id\": \"%s\",\n", b->bill_id);
        fprintf(fp, "      \"consumer_id\": \"%s\",\n", b->consumer_id);
        fprintf(fp, "      \"billing_cycle\": \"%s\",\n", b->billing_cycle);
        fprintf(fp, "      \"billed_units\": %.1f,\n", b->billed_units);
        fprintf(fp, "      \"net_payable\": %.2f,\n", b->net_payable_amount);
        fprintf(fp, "      \"amount_paid\": %.2f\n", b->amount_paid);
        fprintf(fp, "    }%s\n", (i == b_count - 1) ? "" : ",");
    }
    fprintf(fp, "  ]\n");
    fprintf(fp, "}\n");
...
void storage_seed_demo_data(void) {
    ui_header("GRID DEMO DATA SEEDER", "Populate Grid with Realistic Multi-Category Consumers");
    ...
    Consumer c1 = {
        ...
        "Flat 402, Royal Palms, C-Scheme", "MTR-DOM-8812",
        CAT_DOMESTIC, PHASE_SINGLE, 3.0, 5000.0, 3.0, 0.0, 0.0, "2026-01-15", 1, 0
    };
    ...
    Consumer c2 = {
        ...
        "Villa 12, Malviya Green Enclave", "MTR-DOM-4401",
        CAT_DOMESTIC, PHASE_SINGLE, 2.0, 3000.0, 0.0, 450.0, 0.0, "2026-02-10", 1, 0
    };
    customer_add_record(&c2);

    /* 3. Commercial Tech Hub */
    Consumer c3 = {
        "VB-1003", "Apex Cloud Innovation Hub", "+91 141 2789012", "billing@apexcloud.io",
        "Plot 9, Tech City Software Zone", "MTR-COM-9923",
        CAT_COMMERCIAL, PHASE_THREE, 25.0, 50000.0, 10.0, 0.0, 1500.0, "2026-03-01", 1, 0
    };
    customer_add_record(&c3);

    /* 4. Industrial Textile Factory */
    Consumer c4 = {
        "VB-1004", "Imperial Textiles & Weaving Ltd", "+91 9828033221", "admin@imperialtextiles.in",
        "RIICO Heavy Industrial Sector", "MTR-IND-1002",
        CAT_INDUSTRIAL, PHASE_THREE, 75.0, 150000.0, 0.0, 12450.0, 0.0, "2026-01-05", 1, 1
    };
    customer_add_record(&c4);

    /* 5. Agricultural Farm Pump */
    Consumer c5 = {
        "VB-1005", "Greenfield Irrigation Cooperative", "+91 9602054321", "kisan@greenfieldagro.org",
        "Survey 412, Rural Sub-district", "MTR-AGR-3320",
        CAT_AGRICULTURAL, PHASE_THREE, 10.0, 10000.0, 0.0, 0.0, 0.0, "2026-04-12", 1, 0
    };
    customer_add_record(&c5);

    /* 6. Akshar Miyani Net Zero Solar */
    Consumer c6 = {
        "VB-1006", "Akshar Miyani", "+91 9999988888", "akshar.miyani@voltbill.org",
        "Tech Park Avenue, Prime Sector", "MTR-DOM-7700",
        CAT_DOMESTIC, PHASE_SINGLE, 5.0, 8000.0, 5.0, 0.0, 2500.0, "2026-01-01", 1, 0
    };
    customer_add_record(&c6);

    /* Historical Billing Records */
    BillBreakdown b1, b2, b3;
    billing_calculate(&c1, 1000.0, 1280.0, 150.0, 30.0, 1.0, &b1);
    strncpy(b1.billing_cycle, "JUL-2026", 15);
    b1.status = BILL_PAID;
    b1.amount_paid = b1.net_payable_amount;
    billing_add_record(&b1);

    billing_calculate(&c1, 1280.0, 1610.0, 170.0, 35.0, 1.0, &b2);
    strncpy(b2.billing_cycle, "AUG-2026", 15);
    b2.status = BILL_PAID;
    b2.amount_paid = b2.net_payable_amount;
    billing_add_record(&b2);

    billing_calculate(&c1, 1610.0, 1990.0, 180.0, 40.0, 1.0, &b3);
    strncpy(b3.billing_cycle, "SEP-2026", 15);
    b3.status = BILL_PENDING;
    billing_add_record(&b3);

    BillBreakdown b4, b5;
    billing_calculate(&c3, 5000.0, 7400.0, 400.0, 250.0, 1.0, &b4);
    strncpy(b4.billing_cycle, "AUG-2026", 15);
    b4.status = BILL_PAID;
    b4.amount_paid = b4.net_payable_amount;
    billing_add_record(&b4);

    billing_calculate(&c3, 7400.0, 10150.0, 450.0, 300.0, 1.0, &b5);
    strncpy(b5.billing_cycle, "SEP-2026", 15);
    b5.status = BILL_PENDING;
    billing_add_record(&b5);

    BillBreakdown b6;
    billing_calculate(&c4, 12000.0, 18500.0, 0.0, 800.0, 0.88, &b6);
    ...
    BillBreakdown b7;
    billing_calculate(&c5, 2000.0, 2850.0, 0.0, 0.0, 1.0, &b7);
    ...
    BillBreakdown b8;
    billing_calculate(&c6, 3000.0, 3320.0, 310.0, 20.0, 1.0, &b8);
    ...
    storage_save_all();
    storage_export_csv();
    audit_log("DEMO_DATA_SEEDED", "6 consumers, 8 bills, 4 receipts");
