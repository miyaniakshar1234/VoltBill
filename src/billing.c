...
void billing_render_invoice(const BillBreakdown *b, const Consumer *c) {
    ...
    char bmp_path[128];
    snprintf(bmp_path, sizeof(bmp_path), "data/bills/%s_qr.bmp", b->bill_id);
    storage_export_qr_bmp(bmp_path, qr_link);

    ui_card_end(W);
    printf("  " CLR_GREEN "✓ High-resolution print QR image exported: " CLR_WHITE "%s" CLR_RESET "\n", bmp_path);
...
void billing_generate_flow(void) {
    ...
    if (qact == 1) {
        char qr_link[256];
        snprintf(qr_link, sizeof(qr_link), "upi://pay?pa=voltbill.utility@axisbank&pn=VoltBill%%20Utility&am=%.2f&cu=INR&tn=%s",
                 new_bill.net_payable_amount, new_bill.bill_id);
        char sub[128];
        snprintf(sub, sizeof(sub), "Bill: %s ◈ Consumer: %s ◈ Payable: Rs. %.2f",
                 new_bill.bill_id, c->id, new_bill.net_payable_amount);
